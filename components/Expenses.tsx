            ...
            <CardContent className="pt-6">
                <form onSubmit={handleSubmit} className="space-y-4">
                    <div className="space-y-2">
                        ...
                        <input type="text" placeholder="e.g., Monthly Rent" value={description} onChange={(e) => setDescription(e.target.value)} className="w-full bg-primary p-2 rounded-md" required />
                    </div>
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        <div className="space-y-2">
                            <label className="text-sm font-medium text-text-secondary">Amount (€)</label>
                            <input type="number" placeholder="650" value={amount} onChange={(e) => setAmount(e.target.value)} className="w-full bg-primary p-2 rounded-md" required />
                        </div>
                         <div className="space-y-2">
                            <label className="text-sm font-medium text-text-secondary">Start Month</label>
                            <input type="month" value={startDate} onChange={(e) => setStartDate(e.target.value)} className="w-full bg-primary p-2 rounded-md" required />
                        </div>
                         <div className="space-y-2">
                         ...
                         <div className="space-y-2">
                            ...
                            <select value={mode} onChange={(e: React.ChangeEvent<HTMLSelectElement>) => setMode(e.target.value as ExpenseMode)} className="w-full bg-primary p-2 rounded-md">
                                {Object.values(ExpenseMode).map(s => <option key={s} value={s}>{s}</option>)}
                            </select>
                        </div>
                    </div>
        ...
        <div className="space-y-6">
            <div className="flex justify-between items-start">
                <div>
                 ...
                 <div className="flex space-x-2 p-1 bg-surface rounded-lg border border-primary">
                ...
                <CardContent>
                    <ResponsiveContainer width="100%" height={300}>
                        <ComposedChart data={monthlyData} margin={{ top: 5, right: 20, left: 20, bottom: 5 }}>
                            ...
                            <XAxis dataKey="monthLabel" stroke="#a0aec0" />
                            <YAxis stroke="#a0aec0" />
                            <Tooltip cursor={{fill: '#4a5568'}} contentStyle={{backgroundColor: '#2d3748', border: 'none'}}/>
                            <Legend />
                            <Bar dataKey="Actual" fill="#f56565" />
                            <Line type="monotone" dataKey="Planned" stroke="#38b2ac" strokeWidth={2} />
                        </ComposedChart>
                    </ResponsiveContainer>
                </CardContent>
            </Card>
